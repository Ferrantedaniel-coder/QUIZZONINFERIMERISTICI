...
- **Totale avanzato opzionale attivo: 366/462**.

File attualmente collegati:

- `data/paziente-chirurgico-explanations-001.json` → `027.json`.
...
- `data/paziente-chirurgico-explanations-manifest.json` e `001`→`029` — archivio completo delle spiegazioni; `001`→`027` sono attualmente collegati al runtime;
...
Punto di ripresa attuale per Paziente chirurgico: **Terapia `te171`**.
...
4. **Terapia: completato `te1–te170`; proseguire da `te171`** con la stessa architettura non bloccante.
